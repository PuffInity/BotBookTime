...
const nodemailerEnvSchema = z
  ...
  .superRefine((env, ctx) => {
    if (!env.SMTP_PASSWORD && !env.SMTP_PASS) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Потрібно задати SMTP_PASSWORD (або SMTP_PASS для backward compatibility).",
        path: ["SMTP_PASSWORD"],
      });
    }
    ...
    if (!env.MAIL_FROM && !env.SMTP_FROM) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Потрібно задати MAIL_FROM (або SMTP_FROM для backward compatibility).",
        path: ["MAIL_FROM"],
      });
    }
  });

const parsedMailerEnv = nodemailerEnvSchema.parse(process.env);

/**
 * Нормалізована конфігурація mailer:
 * - підтримує нові ключі (`SMTP_PASSWORD`, `MAIL_FROM`)
 * - зберігає backward compatibility (`SMTP_PASS`, `SMTP_FROM`)
 */
export const mailerConfig = {
  ...parsedMailerEnv,
  SMTP_PASSWORD: parsedMailerEnv.SMTP_PASSWORD ?? parsedMailerEnv.SMTP_PASS!,
  MAIL_FROM: parsedMailerEnv.MAIL_FROM ?? parsedMailerEnv.SMTP_FROM!,
};
